...
theorem run_load (L : Layout) (fd : FunDef) (holes : List Hole) (s : St) (m : State)
  ...
  have hnoh : NoHalt L 1 m :=
    NoHalt_cons L 0 m (by rw [hq]; exact pc_ne_halt L q hq8 hqlt) (NoHalt_zero L (step m))
  rw [hstep]
  refine ⟨StInv_scratch L fd holes s m t _ _ ht2 hinv, ?_, ?_, ?_, fun t' ht' => ?_, hnoh⟩
...
theorem run_storeFrom (L : Layout) (fd : FunDef) (holes : List Hole) (s : St) (m2 : State)
    ...
    ∃ ks, StInv L fd holes (s.rset rd v) (stepN ks m2)
        ...
        ∧ (∀ t, (stepN ks m2).rget t = m2.rget t)
        ∧ NoHalt L ks m2 := by
  ...
  | isTrue hrd0 =>
    ...
    exact ⟨0, hinv2, by simp only [stepN_zero]; exact hq,
           ...
           fun t => by rw [stepN_zero], NoHalt_zero L m2⟩
  | isFalse hrd0 =>
    ...
    · exact NoHalt_cons L 0 m2 (by rw [hq]; exact pc_ne_halt L q hq8 hqlt) (NoHalt_zero L (step m2))
...
/-- Run the 3-instruction `synthConst t v` sequence: materialises `ofInt 64 v`
    into scratch register `t` (`t ≠ 0, 2`), IL state `s` unchanged (only `t`/`pc`
    move) and every OTHER register preserved, given `synthHi v` fits the 12-bit
    signed window (the compiler's `cref` guard / data-length bound). The
    other-register clause is what lets `cref` synth `δ` into `T1` without
    clobbering the pc-read it left in `T0`. `[propext, Quot.sound]`. -/
theorem run_synth (L : Layout) (fd : FunDef) (holes : List Hole) (s : St) (m : State)
    (t : Nat) (ht0 : t ≠ 0) (ht2 : t ≠ 2)
    (v : Int) (q : Nat) (hinv : StInv L fd holes s m)
    (hpc : m.pc = L.codeBase + BitVec.ofNat 64 q) (hem : Emitted L q (synthI t v))
  ...
  have hd0 : decode (fetch32 m) = Instr.addi t 0 (BitVec.ofInt 12 (synthHi v)) := by
    have h := decode_at L m m q (synthI t v) hem hinst 0 (by simp) (by simpa using hpc) rfl
    simpa [synthI] using h
  have hs0 : step m = (m.rset t (BitVec.ofInt 64 (synthHi v))).setPc (m.pc + 4) := by
    rw [step_addi m t 0 _ hd0]
  ...
  have hT0_0 : (step m).rget t = BitVec.ofInt 64 (synthHi v) := by
    rw [hs0, rget_setPc]; exact rget_rset_self _ _ _ ht0
  have hd1 : decode (fetch32 (step m)) = Instr.slli t t 12 := by
    have h := decode_at L m (step m) q (synthI t v) hem hinst 1 (by simp) (by rw [hpc0]) hmem0
    simpa [synthI] using h
  have hs1 : step (step m)
      = ((step m).rset t ((BitVec.ofInt 64 (synthHi v)) <<< (12:Nat))).setPc ((step m).pc + 4) := by
    rw [step_slli (step m) t t 12 hd1, hT0_0]
  ...
  have hT0_1 : (step (step m)).rget t = (BitVec.ofInt 64 (synthHi v)) <<< (12:Nat) := by
    rw [hs1, rget_setPc]; exact rget_rset_self _ _ _ ht0
  have hd2 : decode (fetch32 (step (step m))) = Instr.addi t t (BitVec.ofInt 12 (synthLo v)) := by
    have h := decode_at L m (step (step m)) q (synthI t v) hem hinst 2 (by simp) (by rw [hpc1]) hmem1
    simpa [synthI] using h
  ...
  have hs2 : step (step (step m)) = ((step (step m)).rset t (BitVec.ofInt 64 v)).setPc
      ((step (step m)).pc + 4) := by
    rw [step_addi (step (step m)) t t _ hd2, hT0_1,
        signExtend_ofInt_12 _ (by omega) (by omega), ← hval]
  have hI0 : StInv L fd holes s (step m) := by
    rw [hs0]; exact StInv_scratch L fd holes s m t _ _ ht2 hinv
  have hI1 : StInv L fd holes s (step (step m)) := by
    rw [hs1]; exact StInv_scratch L fd holes s (step m) t _ _ ht2 hI0
  have hI2 : StInv L fd holes s (step (step (step m))) := by
    rw [hs2]; exact StInv_scratch L fd holes s (step (step m)) t _ _ ht2 hI1
  refine ⟨?_, ?_, ?_, ?_, ?_,
  ...
  · rw [show stepN 3 m = step (step (step m)) from rfl, hs2, rget_setPc]
    exact rget_rset_self _ _ _ ht0
...
theorem run_cref (L : Layout) (fd : FunDef) (holes : List Hole) (s : St) (m : State)
  ...
  obtain ⟨hinvS, hT1S, hpcS, hmemS, hpres, hnohS⟩ :=
...
theorem single_op_sim {L : Layout} {fd : FunDef} {holes : List Hole}
  ...
  obtain ⟨hinv1, h1pc, h1mem, h1T0, -, -⟩ :=
    run_load L fd holes s m rs T0 pos hinv hrs hfrs (by decide) (by decide) hpc hemL hpos8 (by omega)
  ...
  obtain ⟨ks, hSt, hStpc, hFrStore, hnohS⟩ := run_store L fd holes s (step (step m)) rd vC (pos + 8)
  ...
  · exact NoHalt_chain L 2 ks m hnoh2 (by rw [h2run]; exact hnohS)
...
theorem two_op_sim {L : Layout} {fd : FunDef} {holes : List Hole}
  ...
  obtain ⟨hinv1, h1pc, h1mem, h1T0, -, -⟩ :=
    run_load L fd holes s m r1 T0 pos hinv hr1 hfr1 (by decide) (by decide) hpc hemL0 hpos8 (by omega)
  ...
  have hemS : Emitted L (pos + 12) (storeSlotI rd T0) := by
    ...
    simpa using h
  obtain ⟨ks, hSt, hStpc, hFrStore, hnohS⟩ := run_store L fd holes s (step (step (step m))) rd vC (pos + 12)
  ...
  have hnoh3 : NoHalt L 3 m :=
    NoHalt_cons L 2 m (by rw [hpc]; exact pc_ne_halt L pos hpos8 (by omega))
      (NoHalt_cons L 1 (step m) (by rw [h1pc]; exact pc_ne_halt L (pos + 4) (by omega) (by omega))
        (NoHalt_cons L 0 (step (step m)) (by rw [h2pc]; exact pc_ne_halt L (pos + 8) (by omega) (by omega))
          (NoHalt_zero L _)))
  refine ⟨3 + ks, ?_, ?_, ?_, ?_⟩
  ...
  · exact NoHalt_chain L 3 ks m hnoh3 (by rw [h3run]; exact hnohS)
...
theorem jump_sim (L : Layout) (fd : FunDef) (holes : List Hole) (s : St) (m : State)
  ...
  · exact NoHalt_cons L 0 m (by rw [hpc]; exact pc_ne_halt L here hhere8 hherelt)
      (NoHalt_zero L (step m))
...
theorem sub3_body_sim {P : Program} {dbase : Name → Option Word} {pad : Name → Nat}
  ...
  obtain ⟨k1, hinv1, hpc1, -, -⟩ := two_op_sim s m 6 10 11 pos (.add T0 T0 T1)
    ...
    (fun m' hd hT0 hT1 => by rw [step_add m' T0 T0 T1 hd, hT0, hT1])
  have hlen4 : (emit (LowIR.Prog.Stmt.add 6 10 11)).length = 4 := by decide
